...
export function LoanScenariosTab({ leadId, lead, onActivity }: Props) {
  ...
  useEffect(() => { load(); setAutoTried(false); }, [leadId]);

  // Auto-generate 3 scenarios on first load when none exist and we haven't
  // already auto-generated for this lead. One-shot: a flag on
  // mortgage_profiles.notes prevents re-creation if the LO later deletes them.
  useEffect(() => {
    if (loading || autoTried || autoBusy) return;
    ...
    (async () => {
      ...
      try {
        ...
        let mpNotes: any = {};
        try { mpNotes = mp?.notes ? JSON.parse(mp.notes) : {}; } catch {}
        if (mpNotes?.loan_scenarios_autogenerated_at) return; // one-shot

        const price = Number(mp?.purchase_price ?? lead?.property_value ?? 0);
        if (!price || price <= 0) return; // missing data — show inline notice
        ...
        const nextNotes = { ...mpNotes, loan_scenarios_autogenerated_at: new Date().toISOString() };
        if (mp?.id) {
          await supabase.from("mortgage_profiles")
            .update({ notes: JSON.stringify(nextNotes) }).eq("id", mp.id);
        } else {
          await supabase.from("mortgage_profiles")
            .insert({ lead_id: leadId, notes: JSON.stringify(nextNotes) });
        }
        toast({
          title: "3 scenarios auto-generated from lead data",
          description: warnings.length ? warnings.join(" ") : "Edit any card to refine.",
        });
        await load();
        onActivity?.();
      } catch (e: any) {
        console.error("Auto-generate scenarios failed:", e);
      } finally {
        setAutoBusy(false);
      }
    })();
  }, [loading, scenarios.length, lead?.id, leadId, autoTried, autoBusy, user?.id]);
